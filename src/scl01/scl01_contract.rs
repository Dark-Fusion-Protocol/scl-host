...
impl SCL01Contract {
                    ...
                    for drip in old_drips {
                        let new_drip = Drip {
                            ...
                            amount: drip.amount.clone()
                                - (current_block_height - drip.start_block) * drip.drip_amount,
    ...
    pub fn start_drip(
        ...
        payload: &String,
        sender_utxos: &Vec<String>,
        receivers: &HashMap<String, (u64, u64)>,
        change_utxo: &String,
        current_block_height: u64,
        ...
        for (utxo, drips_on_utxo) in drips.clone() {
            ...
            for mut drip in drips_on_utxo {
                ...
                if current_block == drip.block_end
                    && ((drip.block_end - drip.start_block) + 1) * drip.drip_amount < drip.amount
                {
                    drip_amount +=
                        drip.amount - (drip.block_end - drip.start_block + 1) * drip.drip_amount;
                }
    ...
    pub fn burn(
        ...
        if owners_amount >= *burn_amount {
            ...
            self.owners
                .insert(change_utxo.to_string(), owners_amount - *burn_amount);
        ...
        if new_listing.list_amt <= owners_amount {
            ...
            if change_amt > 0 {
                if self.owners.contains_key(&new_listing.change_utxo) {
                    ...
                    new_owner.1 = new_amount.clone();
                    self.owners
                        .insert(new_listing.change_utxo.to_string(), new_amount);
                } else {
                    new_owner.1 = change_amt.clone();
                    self.owners
                        .insert(new_listing.change_utxo.to_string(), change_amt);
                }
            }
    ...
    pub fn bid(
        ...
        for (i, _) in bids.iter().enumerate() {
            if listings_available.clone().contains_key(&bids[i].order_id) {
                ...
                if bids[i].bid_amount / 10u64.pow(self.decimals as u32) * bids[i].bid_price
                    >= listings_available[&bids[i].order_id].list_amt
                        / 10u64.pow(self.decimals as u32)
                        * listings_available[&bids[i].order_id].price
                {
    ...
    pub fn accept_bid(
        ...
        if bids_available.contains_key(bid_id) {
            ...
            let payload_data = format!(
                "{}-ExtraInfo-{},{},{}",
                payload,
                bid_id,
                bids_available[bid_id].bid_amount,
                bids_available[bid_id].bid_price
            );
    ...
    pub fn fulfil(
        ...
        if fulfillments.clone().contains_key(bid_id) {
            ...
            if self.owners.contains_key(&recievers_utxo) {
                ...
                self.owners
                    .insert(recievers_utxo.to_string(), bid.bid_amount);
    ...
    pub fn cancel_listing(
        ...
        if self.owners.contains_key(&recievers_utxo) {
            ...
            self.owners
                .insert(recievers_utxo.to_string(), canceled_listing.list_amt);
        }
    ...
    pub fn airdop(
        ...
        if pending {
            ...
            match self.owners.get(receiver) {
                Some(&e) => {
                    self.owners
                        .insert(receiver.to_string(), &e + airdrop_amount);
                    owner_amount += e;
                }
                None => {
    ...
    pub fn create_dim_airdrop(
        &mut self,
        txid: &String,
        payload: &String,
        sender_utxos: &Vec<String>,
        pool_amount: &u64,
        step_down_amount: &u64,
        step_period_amount: &u64,
        max_airdrop: &u64,
        min_airdrop: &u64,
        change_utxo: &String,
        single_drop: &bool,
        ...
        if pool_amount > &owners_amount {
            return Err(
                "create_dim_airdrop: pool amount is more than the owned amount".to_string(),
            );
        }
    ...
    pub fn claim_dim_airdrop(
        &mut self,
        txid: &String,
        payload: &String,
        claim_id: &String,
        reciever_utxo: &String,
        pending: bool,
        donater_pub_address: &String,
    ) -> Result<(String, u64, bool), String> {
        let mut diminishing_airdrops = match self.diminishing_airdrops.clone() {
            Some(diminishing_airdrops) => diminishing_airdrops,
            None => {
                return Err(
                    "claim_dim_airdrop: contract has reached no claimable diminsihing airdrops"
                        .to_string(),
                )
            }
        };
        ...
        let mut dim_airdrop: DimAirdrop = match diminishing_airdrops.get(claim_id) {
            ...
            None => {
                return Err("claim_dim_airdrop: diminishing airdrop claim id not found".to_string())
            }
        };
        ...
        if pending {
            ...
            if self.owners.contains_key(reciever_utxo) {
                ...
                self.owners
                    .insert(reciever_utxo.to_string(), airdrop_amount);
            }
        }
        ...
        if dim_airdrop.single_drop {
            dim_airdrop
                .claimers
                .insert(donater_pub_address.to_string(), airdrop_amount);
        }
    ...
    pub fn claim_dge(
        ...
        if donation as u128
            > (dge.max_drop as u128 * dge.sats_rate as u128)
                / 10u64.pow(self.decimals as u32) as u128
        {
        ...
        match self.owners.get(reciever_utxo) {
            Some(&existing_amount) => {
                self.owners
                    .insert(reciever_utxo.to_string(), &existing_amount + drip_amount);
    ...
    pub fn provide_liquidity(
        ...
        if liquidation_amount <= owners_amount {
            ...
            let change_drip_present =
                self.transfer_drips(&owned_senders, block_height, &change_utxo);
            ...
            return Err(
                "provide_liquidity: owner amount is less than the liquidation amount ".to_string(),
            );
        }
    }
    ...
    pub fn liquidate_position(
        ...
        let mut liquidated_tokens = match self.liquidated_tokens.clone() {
            Some(liquidated_tokens) => liquidated_tokens,
            None => {
                return Err("liquidate_position: contract has no lp tokens assigned".to_string())
            }
        };
    ...
    pub fn swap_lp(
        ...
        txid: &str,
        payload: &str,
        sender_contract_id: String,
        provided_amount: u64,
        quoted: u64,
        slipage_tolerance: f32,
    ) -> Result<u64, String> {
        ...
        let mut swap_amount = ((reciever_pool_amount as f64)
            - (liquidity_pool.k as f64 / (sender_pool_amount as f64 + fee_diff)))
            as u64;
        ...
        liquidity_pool
            .swaps
            .insert(txid.to_string(), (provided_amount, swap_amount));
        self.liquidity_pool = Some(liquidity_pool);
        self.payloads.insert(txid.to_string(), payload.to_string());
        return Ok(swap_amount);
    }

    pub fn liquidate_postion_lp(
        &mut self,
        txid: &str,
        payload: &str,
        lp_utxos: &Vec<String>,
        claim_amount: u64,
        block_height: u64,
    ) -> Result<(u64, u64, String, u64, bool), String> {
        let mut liquidity_pool = match self.liquidity_pool.clone() {
            Some(liquidity_pool) => liquidity_pool,
        ...
        if total_tokens >= claim_amount {
            ...
            liquidity_pool
                .liquidations
                .insert(txid.to_string(), (token_1, token_2));
            self.liquidity_pool = Some(liquidity_pool);
            ...
            return Err(
                "liquidate_postion_lp: owner amount is less than the claim amount ".to_string(),
            );
        }
    }
                ...
                if let Some(old_drips) = drips.get(&utxo) {
                    for drip in old_drips {
                        let new_drip = Drip {
                            ...
                            drip_amount: drip.drip_amount.clone(),
                            amount: drip.amount.clone()
                                - (block_height - drip.start_block) * drip.drip_amount,
